...
class Session(object):
    ...
    def connect(self, to_header, routes, streams, is_focus=False, transfer_info=None, extra_headers=None):
        ...
        try:
            ...
            local_sdp = self.generate_sdp(local_ip)
            ...
            # route_header = RouteHeader("127.0.0.1")
            ...
            # print(self.account.uri)
            # exit()
            #file_name = str(self.account.uri) + ".txt"
            ...
            print("from header: " + str(from_header))
            print("to header: " + str(to_header))
            print("route header: " + str(route_header))
            print("contact header: " + str(contact_header))
            print("local_sdp: " + str(local_sdp))   
            print("credentials: " + str(self.account.credentials))
            print("extra_headers: " + str(extra_headers))
            ...
            try:
                with api.timeout(settings.sip.invite_timeout):
                    while True:
                        notification = self._channel.wait()
                        print("Notification: {}".format(notification.name))
                        with open("one.log", "a+") as f:
                            f.write(str(time.time()) + "\n")
                        if notification.name == 'SIPInvitationGotSDPUpdate':
                            if notification.data.succeeded:
                                ...
                                print("------------------------------------------")
                                print("------- Invitation SDP Got Updated -------")
                                print("--------------- Local_sdp ----------------")
                                print(local_sdp)
                                print("--------------- Local_sdp ----------------")
                                print("--------------- Remote_sdp ----------------")
                                ...
                                sip_device_params, _ = parse_sdp(str(local_sdp))
                                rtp_device_params, _ = parse_sdp(str(remote_sdp))
                                
                                self.start_media_stream(sip_device_params, rtp_device_params)
            ...
            # print("media_timeout: =============================")
            # print(self.media_stream_timeout)
            # print("media_timeout: =============================")
            ...
            #         print("notification state: {}".format(notification.data.state))
            #         print("notification sub state: {}".format(notification.data.sub_state))
            #         # print("SIPInvitationChangedState ------------------")
            #         # print("state: =================")
            #         # print(notification.data.state)
            #         # print("state: =================")
            #         # print("sub state: 000000000000")
            #         # print(notification.data.sub_state)
            #         # print("sub state: 000000000000")
            #         if notification.data.state == 'early':
            #             if notification.data.code == 180:
            #                 notification_center.post_notification('SIPSessionGotRingIndication', self)
            ...
            if e.data.prev_state in ('connecting', 'connected') or getattr(e.data, 'method', None) == 'BYE':
                ...
                if e.data.originator == 'remote':
                    ...
                    # : we should know *exactly* when there are set -Saul
    ...
    def _NH_SIPInvitationChangedState(self, notification):
        print("_NH_SIPInvitationChangedState")
        print("notification.data.state: " + str(notification.data.state))
            ...
            try:
                if notification.data.state == 'connected' and notification.data.sub_state == 'received_proposal':
                    ...
                    try:
                        ...
                        # print("proposed_remote_sdp: {}".format(self._invitation.sdp.proposed_remote))
                        # print("proposed_len: {}".format(len(proposed_remote_sdp.media)))
                        # print("active_remote_sdp: {}".format(self._invitation.sdp.active_remote))
                        # print("active_len: {}".format(len(active_remote_sdp.media)))
                        ...
                        #     return
                            ...
                            try:
                                self._invitation.send_response(100)
                                self._invitation.send_response(200, sdp=local_sdp)
                                print("Send Proposal Response")
                                #self.start_media_stream()
                                #print("sending received proposal response!")
                            except PJSIPError:
                                ...
                                while not received_sdp_update or not received_invitation_state or self._channel:
                                    ...
                                    if notification.name == 'SIPInvitationGotSDPUpdate':
                                        ...
                                        if notification.data.succeeded:
                                            ...
                                            _media = remote_sdp.media[0]
                                            send_ip = _media.connection.address.decode()
                                            send_port = _media.port
                                            # print("jiohtjhlktrjhktrlhjtkrlhjklrhjlkthjtklrjkl")
                                            # print("send_ip: {}, send_port: {}".format(send_ip, send_port))
                                            # print("jioooooooooooooooooooooooooooooooooooooooo")
                                            #if (send_port == 0):
                                            # if (send_ip == "0.0.0.0"):    
                                                # self.stop_media_stream()    
                                            # else:
                                            #     self.start_media_stream(send_ip, send_port)

                                            #if remote_sdp.media[0].port = 0
                                            #Hsu
                                            
                                            # self.start_media_stream("140.114.77.83", 12000)
    ...
    def _NH_SIPInvitationGotSDPUpdate(self, notification):
        print("_NH_SIPInvitationGotSDPUpdate")
    ...
    def generate_sdp(self, local_ip):
        ...
        # media = SDPMediaStream("audio".encode(), 11000, "RTP/AVP".encode())
        # media.formats = ['111'.encode()]
        # media.attributes = [SDPAttribute("rtpmap".encode(), "111 opus/48000".encode()), SDPAttribute('sendonly'.encode(), "".encode())]
        # media.connection = conn
        # sdp.media.append(media)
        
        # media = SDPMediaStream("point_cloud".encode(), 12000, "RTP/AVP".encode())
        # media.formats = ['98'.encode()]
        # #media_3.attributes = [SDPAttribute("rtpmap".encode(), "98 octree_compression/90000".encode()), SDPAttribute('sendonly'.encode(), "".encode()), SDPAttribute("fmtp".encode(), "98 profile-level-id=420029; packetization-mode=1; sprop-parameter-sets=Z00AKpY1QPAET8s3AQEBQAABwgAAV+QB,aO48gA==".encode())]
        # media.attributes = [SDPAttribute("rtpmap".encode(), "98 octree_compression".encode()), SDPAttribute('sendonly'.encode(), "".encode())]
        # media.connection = conn
        # sdp.media.append(media)
    ...
    def stop_media_stream(self):
        ...
        try:
            ...
            self.rtp_session.wait(timeout=20)
        except subprocess.TimeoutExpired:
            self.rtp_session.kill()
            self.rtp_session.wait()
        finally:
            print("rtp_session process terminated.")

        # sleep(1)
        # self.media_stream.kill()
        # self.path_proc.kill()
    ...
    def start_media_stream(self, sip_device_params, rtp_device_params):
        ...
        # self.rtp_session = subprocess.Popen(["python3","../../AIoTtalk_plus_Lib/example/sender.py", sip_device_params_json_str, rtp_device_params_json_str])
        self.rtp_session = subprocess.Popen(["python3","../../AIoTtalk_plus_Lib/example/send_data/send_img.py", sip_device_params_json_str, rtp_device_params_json_str])
        print("start_media_stream!")
        # time.sleep(3)
        # print("stop_media_stream!")
        #self.end()
        #exit(0)
        # sleep(1)
        # print("start media stream -----")
        # self.media_stream = subprocess.Popen(["/home/james/Streaming/SIPUA/LidarUA/cmake/MAIN", str(ip), str(port)])
        # sleep(2)
        # print("start airsim path process")
        # self.path_proc = subprocess.Popen(["python3", "/home/james/Streaming/SIPUA/LidarUA/loam_path.py"])
