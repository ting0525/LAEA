...
SelectedDF = []
def ControlChannel(stop_event=None):
...
MAC=get_mac_addr()
thx=None
def register_device(addr, stop_event=None):
    ...
    if thx == None:
        ...
        thx=threading.Thread(target=ControlChannel, args=(stop_event,))     #for control channel
        thx.daemon = True                               #for control channel
        thx.start()                                     #for control channel 


def device_registration_with_retry(URL=None, addr=None, stop_event=None):
    if URL != None:
        csmapi.ENDPOINT = URL
    success = False
    while not success:
        try:
            register_device(addr, stop_event)
            success = True
        except Exception as e:
            print ('Attach failed: '),
            print (e)
