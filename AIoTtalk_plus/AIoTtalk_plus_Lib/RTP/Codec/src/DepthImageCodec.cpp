...
// 壓縮深度圖資料
int DepthImageCodec::encode(Datatype &_raw_data, Binary_Data *_encode_data){    
    if(std::holds_alternative<cv::Mat>(_raw_data)){
        
        const cv::Mat &depth_image = std::get<cv::Mat>(_raw_data);
        ...
        _encode_data->size = compressed_data.size();
        _encode_data->buffer = new uint8_t[_encode_data->size];
        std::copy(compressed_data.begin(), compressed_data.end(), _encode_data->buffer);
...
// 還原深度圖資料
int DepthImageCodec::decode(Binary_Data *_encode_data, std::vector<Datatype> &_decode_data){
    
    std::vector<uint8_t> compressed_data(_encode_data->buffer, _encode_data->buffer + _encode_data->size);
    ...
    cv::Mat restore_depth_image(height, width, CV_32FC1);
    
    std::memcpy(restore_depth_image_uint16.data, restore_depth_data.data(), restore_depth_data.size() * sizeof(uint16_t));
    ...
    _decode_data.push_back(std::move(restore_depth_image));
