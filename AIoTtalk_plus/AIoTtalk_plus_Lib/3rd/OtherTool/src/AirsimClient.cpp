...
void AirsimClient::get_lidar_data(msr::airlib::LidarData &lidar_data, 
    ...
    // change to the airsim setting.json file configuration
    ...
    // for(int i=0; i < 20; i++){
    //     std::cout << "x: " << *iter << " y: " << *(iter+1) << " z: " << *(iter+2) << std::endl;
    //     iter+=3;
    // }
    ...
    //iter = lidar_data.point_cloud.begin();
...
void AirsimClient::process_depth_data(cv::Mat &depth_image, cv::Mat &depth_image_uint16){
    ...
    //std::cout << "is_nan: " << is_nan_num << std::endl;
    //std::cout << "excceed_value: " << is_excceed_value_num << std::endl;
    //std::cout << "normal_value: " << is_normal_value_num << std::endl;
}