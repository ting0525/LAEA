...
class SIPAudioApplication(SIPApplication):
    ...
    def _NH_SIPApplicationDidStart(self, notification):
        ...
        if self.target is not None:
            ...
            try:
                ...
                self.output.put('DNS lookup for %s' % uri)
    ...
    def _NH_SIPSessionNewIncoming(self, notification):
        ...
        print("rjtijritjritjritjritjritjrit")
        print(session.remote_identity.uri)
        ...
        #print("==========================================")
        #print(self.incoming_sessions)
        #print(len(self.incoming_sessions))
        #print("==========================================")
        #return
    ...
    def _NH_SIPSessionDidStart(self, notification):
        ...
        thread = Thread(target = self.recording_thread, args=(session, ))
        thread.daemon = True
        thread.start()
        print("start recording_thread")
        return 
        ...
        # userName = session.remote_identity.uri.user
        # filename = userName + "_0.wav"

        #os.system("gnome-terminal -e 'python3 sip_audio_session.py'") 
        # record_dir = "/home/james/AIoTtalk_M/AI_Cli/record"
        # path = os.path.join(record_dir, "siptalk_audio")
        # audio_stream.start_recording(os.path.join(path, filename))
        
        # self.EndCall_flag = False
        # rec_timer_thread=Thread(target = self.rec_timer)
    ...
    def recording_thread(self, session):

        device_name = session.remote_identity.uri.user
        print("Device Name: " + device_name)

        audio_stream = session.streams[0]
        print(audio_stream)
        filename_no = 0
        save_file_directory = "/home/james/AIoTtalk_M/AI_Cli/record/"
        direction = session.direction
        
        while(True):
            if self.EndCall_flag is not True:
                save_file_path = save_file_directory + device_name + "_" + str(filename_no) + ".wav"
                audio_stream.start_recording(save_file_path)
                sleep(2)
                audio_stream.stop_recording()
                ...
                self.audio_file_queue.put(data)
            else:
                break
...
if __name__ == '__main__':
    ...
    target = None
    options_dict = {
		'account': "7001@140.114.77.83", 
		'config_directory': None, 
		'trace_sip': False, 
		'trace_pjsip': False, 
		'trace_notifications': False, 
		'disable_sound': False, 
		'auto_answer_uris' : None, 
		'auto_answer_interval': 0,
		'auto_hangup_interval': None,
		'external_id' : None,
		'spool_dir' : None,
		'enable_default_devices' : None,
		'enable_video' : False,
		'batch_mode': False,
		'daemonize' : False
		}
