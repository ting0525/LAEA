...
class AICli_SIPApplication(SIPApplication):
    def __init__(self):
        self.account = None
    ...
    def _NH_SIPAccountRegistrationDidSucceed(self, notification):
        ...
        if self.registration_succeeded:
            print("account_registration succeeded")
            return
    ...
    def recording_thread(self, session):
        ...
        # sip:device@xxx.xxx:5566
        device_uri = str(session.remote_identity.uri)
        device_uri = device_uri.split(":")[1]
        print(device_uri)
        print("Device URI: " + str(device_uri))

        audio_stream = session.streams[0]
        print(audio_stream.recorder)
        filename_no = 0
        save_file_directory = os.getcwd() + "/record/"
        direction = session.direction
        
        while(True):
            save_file_path = save_file_directory + device_uri + "_" + str(filename_no) + ".wav"
...
if __name__ == '__main__':
    sip_session_application = SIPSessionApplication("7001@140.114.77.83")
    sip_session_application.start()
