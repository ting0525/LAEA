...
class Session(object):
    ...
    def accept(self, streams, is_focus=False, extra_headers=None):
        ...
        try:
            ...
            from_sip_account = self._invitation.from_header
    ...
    def _NH_SIPInvitationGotSDPUpdate(self, notification):
        print("sipinvitationgotsdpupdate!")
        if self.greenlet is not None:
            self._channel.send(notification)
    ...
    def _NH_RTPStreamDidEnableEncryption(self, notification):
        print("RTPStreamdidEnable Encryption")
    ...
    @transition_state('incoming', 'accepting')
    @run_in_green_thread
    def my_accept(self, mac, device_feature):

        '''send 100 trying and 180 ringing response'''
        self._invitation.send_response(100)
        self._invitation.send_response(180)
        
        '''push SDP to RTP Establisher SDP Parser''' 
        remote_sdp = self._invitation.sdp.proposed_remote
        sip_header = self._invitation.from_header
        # device_sip_account = re.search(r'([\w.-]+@[\w.-]+)', str(sip_header))
        # device_sip_account = device_sip_account.group(0)
        data = [str(self.sip_device_id), str(remote_sdp).replace("\r", "")]
        self.push_to_SDP_Parser(mac, device_feature, data)

        # print("---remote sdp---")
        #import sdp_transform
        #sdp_dict = sdp_transform.parse(str(remote_sdp))
        #print(sdp_dict)
        # print(remote_sdp)
        # return
    ...
    #def push_to_SDP_Parser(self, mac, device_model, device_feature, data):
    def push_to_SDP_Parser(self, mac, device_feature, data):
        ...
        if(response.status_code != 200):
            ...
            self._fail(originator='local', code=0, reason=None, error='IoTtalk push failed')

    @run_in_green_thread
    def init_session(self, sdp):
        self.greenlet = api.getcurrent()
        notification_center = NotificationCenter()
        settings = SIPSimpleSettings() 
        ...
        try:
            contact_header = ContactHeader.new(self._invitation.local_contact_header)
            self._invitation.send_response(200, contact_header=contact_header, sdp=sdp, extra_headers=extra_headers)
            notification_center.post_notification('SIPSessionWillStart', sender=self)
            ...
            while True:    
                ...
                if notification.name == 'SIPInvitationGotSDPUpdate':
                    ...
                    print("SIP invitation changed state")
                    if notification.data.state == 'early':
                        print("invitation state early ---------")
                    if notification.data.state == 'connecting':
                        print("invitation state connecting ---------")
                    if notification.data.state == 'connected':
                        print("invitation state connected ---------")
                        if not connected:
                            connected = True
                            notification_center.post_notification('SIPSessionDidProcessTransaction', self, NotificationData(originator='remote', method='INVITE', code=200, reason=sip_status_messages[200], ack_received=True))
                        elif notification.data.prev_state == 'connected':
            ...
            print("InvitationDisconnectedError raised!")
            notification_center.remove_observer(self, sender=self._invitation)  
            ...
            self.proposed_streams = None
            self.start_time = ISOTimestamp.now()
            notification_center.post_notification('SIPSessionDidStart', self, NotificationData(streams=[]))
        finally:
            self.greenlet = None
        
        self.is_connected = True
        print("Session Initialization Finished !")
    
    @run_in_green_thread
    def terminate_session(self, mac, device_feature):
        sip_header = self._invitation.from_header
        # device_sip_account = re.search(r'([\w.-]+@[\w.-]+)', str(sip_header))
        # device_sip_account = device_sip_account.group(0)

        data = [str(self.sip_device_id), "terminated"]
        # print("---remote sdp---")
        # print(remote_sdp)
        # return
        self.push_to_SDP_Parser(mac, device_feature, data)
    ...
    @run_in_green_thread
    def update_sdp(self, sdp):
        self.state = 'sending_proposal'
        self.greenlet = api.getcurrent()
        notification_center = NotificationCenter()
        settings = SIPSimpleSettings()

        self._invitation.send_reinvite(sdp=sdp)
                    ...
                    while not received_invitation_state or not received_sdp_update:
                        ...
                        if notification.name == 'SIPInvitationGotSDPUpdate':
                            ...
                            if notification.data.state == 'connected' and notification.data.sub_state == 'normal':
                                ...
                                notification_center.post_notification('SIPSessionDidProcessTransaction', self, NotificationData(originator='local', method='INVITE', code=notification.data.code, reason=notification.data.reason))
                                if notification.data.code >= 300:
                                    print("Reinvite for SDP Update failed")
                                    self.state = 'connected'
                        
                                    notification_center.post_notification('SIPSessionProposalRejected', self, NotificationData(originator='local', code=notification.data.code, reason=notification.data.reason, proposed_streams=[]))
                                    return
                            elif notification.data.state == 'disconnected':
                                raise InvitationDisconnectedError(notification.sender, notification.data)
        
        except InvitationDisconnectedError:
            print("InvitationDisconnectedError raised!")
            notification_center.remove_observer(self, sender=self._invitation)

        except api.TimeoutError:
            print("Time out for reinvite!")
            return
        else:
            return    
