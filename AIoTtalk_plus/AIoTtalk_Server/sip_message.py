...
class SIPMessageApplication(SIPApplication):
    ...
    def listen(self):
        while(True):
            ...
            sys.stdin.flush()
            #res = result.decode().replace("\n", " "))
            s = type(result)
            #print(s)
            x = str(s) + "\n"
            #s = "heeloo"
            self.start_send_message(
                "deviceatest1@140.114.77.83", result, None
            )
            self.start_send_message(
                "deviceatest2@140.114.77.83", result, None
            )
            #with open("uiuiuiuiu", "a") as f:
            #    f.write(result)
            #sys.stdout.write("jgijrigjrigjij\n")
            #sys.stdout.flush()
    ...
    def _NH_SIPApplicationWillStart(self, notification):
        ...
        if self.sip_account is None:
            ...
            possible_accounts = [account for account in account_manager.iter_accounts() if self.sip_account in account.id and account.enabled]
            #print("Matched Account: ", possible_accounts[0])
            if len(possible_accounts) > 1:
    ...
    def _NH_SIPEngineGotMessage(self, notification):
        ...
        #print("Got %s MESSAGE from %s: \n%s\n" % (content_type, identity, content.decode().strip()))
...
if __name__ == "__main__":
    a = SIPMessageApplication()
    #a.listen()
    
    a.start("siptalktest@140.114.77.83", "./sip_accounts")
    a.wait_for_initialization()
    a.wait_for_account_registration()
    ...
    #    s = "hello"
    #a = DNSLookup()
    #a.ookup_sip_proxy("devicetest10@140.114.77.73", settings.sip.transport_list, tls_name=self.account.sip.tls_name or uri.host)