...
namespace fast_planner
{
  ...
  void FastExplorationManager::initialize(ros::NodeHandle &nh)
  {
    ...
    // middle yaw 
    nh.param("exploration/max_yaw_gap", max_yaw_gap, 120);
    nh.param("exploration/lens_lb_rate", lens_lb_rate, 2.0);
    nh.param("exploration/yaw_rotate_rate", yaw_rotate_rate, 1.3);
    nh.param("exploration/temp_next_yaw_thresh", temp_next_yaw_thresh, 2.5);
      ...
      const Vector3d &pos, const Vector3d &vel, const Vector3d &acc, const Vector3d &yaw)
  {
    ...
    if (ed_->points_.size() > 1)
    {
      ...
      { 
        if ((ed_->refined_points_[i] - pos).norm() < 3.0 && !planner_manager_->path_finder_->judgePointsCollision(ed_->refined_points_[0], ed_->refined_points_[i]))
        { 
          ...
          if (angle < max_yaw_gap && angle > 5)
          {
            ...
            // normal_area
              ...
              yawList_normal.push_back(refined_yaws[i]);
          }
        }
      }
      if (frontier_num >= 1)
    ...
    { 
      ...
      double temp_time_lb, temp_time_ratio;
      double temp_diff2, temp_diff_lb2;
      double temp_diff, temp_diff_lb;

      double len_vel_lb = len * lens_lb_rate / vel.norm();
      // double len_vel_lb = len * 1.5 / vel.norm();

      if(yawList.size()){
        for (int i = 0; i < yawList.size(); i++)
        { 
          temp_diff = fabs(yawList[i] - yaw[0]);
          temp_diff_lb = min(temp_diff, 2 * M_PI - temp_diff);
          temp_diff2 = fabs(next_yaw - yawList[i]);
          temp_diff_lb2 = min(temp_diff2, 2 * M_PI - temp_diff2);

          temp_time_lb = (temp_diff_lb + temp_diff_lb2) * yaw_rotate_rate / ViewNode::yd_;
          temp_time_ratio = temp_diff_lb / (temp_diff_lb + temp_diff_lb2);

          if(temp_time_lb < len_vel_lb){
            
            if (temp_diff_lb > time_lb)
            {
              time_lb = temp_time_lb;
              time_ratio = temp_time_ratio;
              select_next = 1;
              middle_yaw = yawList[i];
            }
          }
        }
      }
      else if(yawList_normal.size()){
        for (int i = 0; i < yawList_normal.size(); i++)
        { 
          temp_diff = fabs(yawList_normal[i] - yaw[0]);
          temp_diff_lb = min(temp_diff, 2 * M_PI - temp_diff);
          temp_diff2 = fabs(next_yaw - yawList_normal[i]);
          temp_diff_lb2 = min(temp_diff2, 2 * M_PI - temp_diff2);

          temp_time_lb = (temp_diff_lb + temp_diff_lb2) * 1.3 / ViewNode::yd_;
          temp_time_ratio = temp_diff_lb / (temp_diff_lb + temp_diff_lb2);
          ...
          if(temp_time_lb < len_vel_lb){
            ...
            if (temp_diff_lb > time_lb)
            {
              ...
              time_ratio = temp_time_ratio;
              select_next = 1;
              middle_yaw = yawList_normal[i];
            }
          }
        }
      }
      
      if(!select_next || time_ratio == 1 || time_ratio == 0){
        manyViewPoints = false;
    ...
    if(len > temp_next_yaw_thresh){  
      ...
      Vector3d yaw_dir{cos(next_yaw), sin(next_yaw), 0};  // 

      auto info = &planner_manager_->local_data_;
      double traj_duration = info->duration_;
     
      Eigen::Vector3d temp_pos_0, temp_pos_1, dir_pose{0,0,0};
      for (double i = 0.1; i <= traj_duration; i = i + 0.1)
      {
        temp_pos_0 = info->position_traj_.evaluateDeBoorT(i-0.05);
        temp_pos_1 = info->position_traj_.evaluateDeBoorT(i);
        dir_pose += (temp_pos_1-temp_pos_0);
      }
      dir_pose(2) = 0;
      dir_pose = dir_pose.normalized();
      
      double angle_path = atan2(dir_pose(1), dir_pose(0));

      double angle_gap = fabs(acos(yaw_dir.dot(dir_pose)));
      if (angle_gap > M_PI)
        angle_gap = 2 * M_PI - angle_gap;

      cout << "Path_dir: " << dir_pose(0) << ", " << dir_pose(1) << std::endl;
      cout << "Yaw_dir: " << yaw_dir(0) << ", " << yaw_dir(1) << std::endl;
      ROS_WARN("Pay Attention to uav movements");
      cout << "Next: " << next_yaw << "Path: " << angle_path <<" Angle_gap: " << angle_gap << std::endl;

      if(angle_gap > 1.57){
        next_yaw = angle_path;
        ROS_ERROR("Fix next_yaw to avg_path_dir !");
      }
    }
    ...
    if (manyViewPoints)
    {
      ...
      cout << bold << green << "11111 Two stage yaw planning success 11111" << reset << endl;
    }
    else
    { 
