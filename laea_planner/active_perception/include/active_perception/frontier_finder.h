...
struct Frontier {
  ...
  int isolateArea = false;  
  int lidar_gains_;
  // The average length of space that can be observed additionally
  double avg_gains_;
  double one_raycast;

  Vector3d extend_pos_;
};
...
public:
  ...
  shared_ptr<Lidar_gain> lidar_gain_;
  ros::Subscriber map_sub_;
  ros::Publisher ray_cast_pub;
  void Occ_MapCallback(const nav_msgs::OccupancyGrid::ConstPtr& msg);
  void displaySthWithColor(vector<Eigen::Vector3d> path, double resolution, Eigen::Vector4d color,
                          int id);
  // draw boundingbox_unknown
  void displayBboxWithColor(vector<Eigen::Vector3d> path, double resolution, Eigen::Vector4d color,
                          int id);

  bool enable_boundary_cost;
...
private:
  ...
  double resolution_2d_cell;
  double lidar_gain_rate;
  // only use max_range_lidar
  double max_range_lidar,max_range_depth,min_range_depth,h_fov_depth,subdivide_rate;
  int count_Lidar_VisibleCells(const Eigen::Vector3d& pos, const double& yaw, const vector<Eigen::Vector3d>& cluster, int& ray_num, Eigen::Vector3d& avg_dir);
  int count_Lidar_VisibleCells(const Eigen::Vector3d& pos, const double& yaw, const vector<Eigen::Vector3d>& cluster, int& ray_num);
  inline bool points_in_resolution(Eigen::Vector3d pos1,Eigen::Vector3d pos2); 

  int count_Lidar_gains(const Eigen::Vector3d& pos, float max_range, const vector<Eigen::Vector3d>& cluster_2d);

  int countVisibleCells_lidar(const Vector3d& pos, const double& yaw, const vector<Vector3d>& cluster);

  int one_Lidar_VisibleCells(const Eigen::Vector3d& pos, const Eigen::Vector3d& avg, float max_range);

  bool Is_Point_in_Rectangle(Vector3d A, Vector3d B, Vector3d C, Vector3d D, Vector3d x);

  std::mutex mtx_plan_;

  vector<Eigen::Vector3d> Bounding_Box_Unknown_;
  vector<vector<Eigen::Vector3d>> BBox_Unknown_Real;
  bool update_BBox_Unknown_ = false;
  ros::Publisher marker_bb_pub;

  // tuning params
  double isolateArea_gain_;
  double small_area_thresh; 
  double small_thresh_low; 

  // tuning params
  double w_angleRate;
  double w_smallareaRate;
  double w_isoareaRate;
  double w_edgeRate;
