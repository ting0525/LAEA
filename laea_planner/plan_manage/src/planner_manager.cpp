...
namespace fast_planner
{
  ...
  bool FastPlannerManager::kinodynamicReplan(
      ...
      const Eigen::Vector3d &end_pt, const Eigen::Vector3d &end_vel, double &time_first, const double &time_lb)
  {
    ...
    if (use_am_traj)
    {
      ...
      time_first = time_duration_list[0];
  ...
  void FastPlannerManager::updateTrajInfo()
  {
    ...
    //   local_data_.first_duration = local_data_.duration_;
    // else
    //   local_data_.first_duration = first_duration;
  ...
  // case YAWING -> yawingBeforeFlight -> here
  //  |start_yaw-end_yaw|=2pi     time_ratio=7.2
  // 对于yawing， start_yaw只有 start_yaw[0]有值，其他为0
  void FastPlannerManager::planYawForYawing(const Eigen::Vector3d &start_yaw, const double &end_yaw, bool lookfwd, const double &relax_time, const double &time_ratio)
  {
    ...
    // for yawing, states2pts(3x3) below,
    // 1.0, -0.6, 0.12
    // 1.0, 0.0, -0.03
    // 1.0, 0.6, 0.12
    // given start_yaw3d: [x 0 0], got
    // yaw.block<3, 1>(0, 0) = x; x; x
    ...
    // calcNextYaw(last_yaw, end_yaw3d(0));
    yaw.block<3, 1>(seg_num, 0) = states2pts * end_yaw3d;
    // yaw.block<3, 1>(12, 0) = y; y; y
    // 从第 seg_num 行开始的大小为 3x1 的子矩阵，也就是最终状态的位置。
    // got yaw: [x;x;x;0;....;0;y;y;y]
    ...
    bspline_optimizers_[1]->setBoundaryStates(start, end);
    // bspline_optimizers_[1]->setWaypoints(waypts, waypt_idx);
    // for yawing  yaw: [x;x;x;0;....;0;y;y;y]  0.6  
    // yaw-控制点 dt_yaw-时间间隔 
    bspline_optimizers_[1]->optimize(yaw, dt_yaw, cost_func, 1, 1);
